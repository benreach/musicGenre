...
const DashboardUsers = () => {
  ...
  return (
    <div className="w-full p-4 flex items-center justify-center flex-col">
      <div
        className="relative w-full py-12 min-h-[400px] overflow-x-scroll my-4 flex flex-col items-center justify-start p-4
      border border-gray-300 rounded-md gap-3"
      >
        <div className="absolute top-4 left-4">
          <p className="text-sm font-semibold">
            Count : <span>{allUsers?.length}</span>
          </p>
        </div>
  ...
  return (
    ...
    >
      {data._id !== user?.user._id && (
        <motion.div
          whileTap={{ scale: 0.75 }}
          className="absolute left-4 w-8 h-8 rounded-md flex items-center 
      ...
      <div className="w-275 min-w-[160px] text-center flex items-center justify-center gap-6">
        ...
        {data._id !== user?.user._id && (
          <motion.p
            whileTap={{ scale: 0.75 }}
            className="text-[10px] font-semibold text-textColor px-1 bg-purple-200 hover:shadow-md rounded-md"
        ...
        {isUserRoleUpdated && (
          ...
          >
            <p>
              Promote <span className="text-red-500">{data.name}</span> to be
              <span className="text-green-600">
                {data.role === "admin" ? " a Member" : " an Admin"}
              </span>
            </p>
            <div className="flex items-center gap-4">
              <motion.button
                whileTap={{ scale: 0.75 }}
                className="outline-none border-none text-sm px-4 py-1 rounded-md
               bg-blue-50 text-black shadow-md"
                onClick={() =>
                  updateUserRole(
                    data._id,
